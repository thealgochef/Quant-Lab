#!/usr/bin/env bash
# UI-2 final gate: the COMPLETE suite as ONE invocation (warnings as errors per
# pyproject), detached from the tool harness (see the release convention).
cd /c/Users/gonza/Documents/Claude-Quant-Lab || exit 1
EV="QL-FSM-PROP-SEARCH-DASHBOARD/implementation-progress/UI-UX-REDESIGN-PLAN/UI-2"
export PY_COLORS=0
echo "full_suite_started $(date -u +%Y-%m-%dT%H:%M:%SZ)" > "$EV/gates.progress"
{
  echo "# UI-2 full suite — start $(date -u +%Y-%m-%dT%H:%M:%SZ)"
