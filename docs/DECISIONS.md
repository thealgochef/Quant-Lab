...
D-044 (supervised ladder + regime algorithm registry + spectral restrictions, V1 = prevalence/logistic/CatBoost/KMeans) lands with R5/R6; the pipeline-runner half of D-045 lands with R5 — all inside `ifvg_prop_robust_config_search_v1`, per the approved final plan package. (D-041 landed with R3, and the trader-workspace half of D-045 with R4, above.)
