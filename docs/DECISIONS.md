...
## Reservation note (2026-08-18, updated 2026-08-21 — not rulings)

The KMeans regime half of D-044 lands with R6; D-042/D-043 remain as landed; the MBP-1 activation block (owner ruling R-6, `research_only_offline`) lands with R5B — all inside `ifvg_prop_robust_config_search_v1`, per the approved final plan package. (D-041 landed with R3; the trader half of D-045 with R4 and its pipeline half with R5, above.)
