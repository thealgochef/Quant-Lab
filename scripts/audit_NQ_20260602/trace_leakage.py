...
from strategy_core import Direction, build_zones, detect_touches
...
from alpha_lab.agents.data_infra.ml.engine_decision import (
    TRADE_TICK, bars_et_to_engine, levels_to_engine, _trade_price_at, _query_trades, _query_quotes)
...
def trace(date_str, max_touches=3):
    ...
    touches = detect_touches(eng_bars, zones := build_zones(levels_to_engine(levels)),
                             tick_size=TRADE_TICK, trading_day=td)
    store = TickStore(DATA_DIR); store.register_symbol_date(SYM, date_str)
    ...
    for tch in touches:
        ...
        aq = _query_quotes(store, SYM, appr_start, tch.bar_ts_utc)
        itk = store.query_tick_feature_rows(SYM, tch.bar_ts_utc, tch.bar_ts_utc + timedelta(minutes=5), price_source="trade")
        entry = _trade_price_at(store, SYM, dts)
        # entry print ts: most-recent <=dts
        fwd = [b for b in eng_bars if b.close_ts_utc > dts and b.close_ts_utc < pd.Timestamp(f"{date_str} 16:15", tz=ET)]
        print(f"\n=== touch {tch.level_type} {tch.direction.value} @ {tt} (decision {dts_et}) ===")
        print(f"  C1 approach trades: n={len(atr)} ts[min..max]={pd.to_datetime(atr.ts_event.min())}..{pd.to_datetime(atr.ts_event.max())}  (must be < touch {pd.Timestamp(tch.bar_ts_utc)})  OK={pd.to_datetime(atr.ts_event.max())<pd.Timestamp(tch.bar_ts_utc).tz_localize('UTC') if atr.ts_event.max().tzinfo is None else pd.to_datetime(atr.ts_event.max())<pd.Timestamp(tch.bar_ts_utc)}")
        print(f"  C2 interaction ticks: n={len(itk)} ts[min..max]={pd.to_datetime(itk.ts_event.min())}..{pd.to_datetime(itk.ts_event.max())}  (must be <= decision {pd.Timestamp(dts)})")
        print(f"  C4 entry print: price={entry} (most-recent trade <= decision {pd.Timestamp(dts)})")
        print(f"  C3 first forward bar close: {pd.Timestamp(fwd[0].close_ts_utc) if fwd else None}  (must be > decision {pd.Timestamp(dts)})")
        shown += 1
