...
def build_bundle(
    ...
    fd = (
        primary_bars.index[0].strftime("%Y-%m-%d")
        if hasattr(primary_bars.index[0], "strftime")
        else "2026-01-05"
    )
    ld = (
        primary_bars.index[-1].strftime("%Y-%m-%d")
        if hasattr(primary_bars.index[-1], "strftime")
        else "2026-02-20"
    )
    ...
    return DataBundle(
        ...
        sessions=[
            SessionMetadata(
                session_id=f"{instrument}_{fd}_RTH",
                session_type="RTH",
                killzone="NEW_YORK",
                rth_open=f"{fd}T09:30:00-05:00",
                rth_close=f"{fd}T16:15:00-05:00",
            )
        ],
        pd_levels={
            fd: PreviousDayLevels(
                pd_high=float(primary_bars["high"].iloc[:n_first_day].max()),
                pd_low=float(primary_bars["low"].iloc[:n_first_day].min()),
                pd_mid=float(
                    (
                        primary_bars["high"].iloc[:n_first_day].max()
                        + primary_bars["low"].iloc[:n_first_day].min()
                    )
                    / 2
                ),
...
def fetch_live_data(symbol: str, days: int = 30) -> pd.DataFrame:
    ...
    if bars.empty:
        ...
        st.stop()
    bars["session_id"] = [f"{symbol}_{ts.strftime('%Y-%m-%d')}_RTH" for ts in bars.index]
    return bars
...
def fetch_databento_data(
    ...
    **No Databento API calls are made.**
    ...
    for i_dt, dt in enumerate(available_dates):
        ...
        if include_ticks:
            ...
            if pq.exists():
                ...
                _conn.execute(f"CREATE VIEW _t AS SELECT * FROM read_parquet('{pq.as_posix()}')")
    ...
    if tick_frames:
        ...
        raw_ticks = raw_ticks[
            (raw_ticks["price"] > median_px * 0.5) & (raw_ticks["price"] < median_px * 1.5)
        ]
...
def simulate_trades(
    ...
    return (
        pd.DataFrame(trades, columns=cols)
        if trades
        else pd.DataFrame(
            columns=cols,
        )
...
def run_pipeline(
    ...
    demo_mode = False
    if not exec_report.approved_signals and not exec_report.vetoed_signals and val_report.verdicts:
        demo_mode = True
    ...
    if exec_report.approved_signals:
        ...
        market_data = {
            "ema_values": [
                float(bars_primary["close"].iloc[-1]),
                float(bars_primary["close"].iloc[-20]),
                float(bars_primary["close"].iloc[-50]),
            ],
            "kama_slope": 0.3,
            "atr_current": float(
                bars_primary["high"].iloc[-20:].mean() - bars_primary["low"].iloc[-20:].mean()
            ),
            "atr_avg": float(bars_primary["high"].mean() - bars_primary["low"].mean()),
            "adx": 25.0,
        }
    ...
    for sv in signal_bundle.signals:
        ...
        if long_pos:
            fig.add_trace(
                go.Scatter(
                    x=long_pos,
                    y=[float(display["low"].iloc[i]) - price_range * 0.02 for i in long_pos],
                    mode="markers",
        ...
        if short_pos:
            fig.add_trace(
                go.Scatter(
                    x=short_pos,
                    y=[float(display["high"].iloc[i]) + price_range * 0.02 for i in short_pos],
                    mode="markers",
        ...
        if entry_x:
            fig.add_trace(
                go.Scatter(
                    ...
                    hovertemplate=("ENTRY %{customdata[0]}<br>Price: %{y:.2f}<extra></extra>"),
                ),
                row=1,
                col=1,
            )
        ...
        if win_x:
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="markers",
                    marker=dict(
                        symbol="star", size=14, color="#26a69a", line=dict(width=2, color="black")
                    ),
                    name="Exit (Win)",
                    customdata=[[p] for p in win_pnls],
                    hovertemplate=("WIN $%{customdata[0]:+.2f}<br>Price: %{y:.2f}<extra></extra>"),
                ),
                row=1,
                col=1,
            )
        ...
        if loss_x:
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="markers",
                    marker=dict(
                        symbol="x", size=12, color="#ef5350", line=dict(width=3, color="#ef5350")
                    ),
                    name="Exit (Loss)",
                    customdata=[[p] for p in loss_pnls],
                    hovertemplate=("LOSS $%{customdata[0]:+.2f}<br>Price: %{y:.2f}<extra></extra>"),
                ),
                row=1,
                col=1,
            )
...
def chart_signal_strength(
    ...
    for sv in signal_bundle.signals:
        ...
        colors = [
            "#26a69a" if v > 0 else "#ef5350" if v < 0 else "#555555"
...
def _strategy_workspace() -> None:
    ...
    with st.sidebar:
        ...
        days = st.slider(
            ...
            7,
            90,
            10 if "Databento" in mode else 30,
        ...
        chart_tf = st.selectbox(
            "Chart Timeframe", tf_options, index=tf_options.index("5m") if "5m" in tf_options else 0
        )
        ...
        st.caption("Primary runtime hook for extrema models saved from the ML Training tab.")
    ...
    st.caption(
        f"{symbol}  |  {mode}  |  {chart_tf}  |  {datetime.now().strftime('%Y-%m-%d %H:%M')}"
    )
    ...
    if not has_results:
        ...
        with tab_chart:
            ...
            if n_tfs > 1:
                st.caption(
                    f"Timeframes loaded: {', '.join(sorted(bars_dict.keys()))} "
                    f"| Showing: {r['chart_tf']}"
                )

            selected = st.selectbox("Select Signal", signal_ids, key="sig_select_chart")
            ...
            with c2:
                st.metric("Current Price", f"${bars_primary['close'].iloc[-1]:,.2f}")
            with c3:
                pct = (bars_primary["close"].iloc[-1] / bars_primary["close"].iloc[0] - 1) * 100
                st.metric("Period Return", f"{pct:+.2f}%")
            with c4:
                st.metric("Signals", signal_bundle.total_signals)
            with c5:
                st.metric("Quality", "PASS" if r["bundle"].quality.passed else "FAIL")

        # ══════════════════════════════════════════════════════════
        # TAB 2: TRADE LOG
        # ══════════════════════════════════════════════════════════
        with tab_trades:
            sel_trade = st.selectbox("Select Signal", signal_ids, key="sig_select_trades")
            ...
            if trades_df.empty:
                ...
                # Equity curve
                st.plotly_chart(chart_equity_curve(trades_df), use_container_width=True)
                ...
                avg_win = (
                    trades_df.loc[trades_df["net_pnl"] >= 0, "net_pnl"].mean() if n_wins > 0 else 0
                )
                avg_loss = (
                    trades_df.loc[trades_df["net_pnl"] < 0, "net_pnl"].mean() if n_losses > 0 else 0
                )

                with c1:
                    st.metric("Total Trades", len(trades_df))
                with c2:
                    st.metric("Net P&L", f"${total_pnl:+,.2f}", delta=f"${total_pnl:+,.2f}")
                with c3:
                ...
                fig_dist.add_trace(
                    go.Histogram(
                        ...
                        name="Net P&L",
                        marker_color=[
                            "#26a69a" if x >= 0 else "#ef5350" for x in trades_df["net_pnl"]
                        ],
                    )
                )
        ...
        with tab_val:
            ...
            with c4:
                st.metric("Bonferroni", "Yes" if val_report.bonferroni_adjusted else "No")
            ...
            for v in val_report.verdicts:
                with st.expander(
                    ...
                    with col_radar:
                        st.plotly_chart(chart_validation_radar(v), use_container_width=True)
                    with col_detail:
                        st.markdown("**Test Results**")
                        st.markdown(f"- IC: `{v.ic:.4f}` (t={v.ic_tstat:.2f})")
                        st.markdown(
                            f"- Hit Rate: `{v.hit_rate:.1%}` "
                            f"(L={v.hit_rate_long:.1%}, S={v.hit_rate_short:.1%})"
                        )
                        st.markdown(f"- Sharpe: `{v.sharpe:.2f}` | Sortino: `{v.sortino:.2f}`")
                        st.markdown(
                            f"- Max DD: `{v.max_drawdown:.1%}` | PF: `{v.profit_factor:.2f}`"
                        )
                        st.markdown(
                            f"- Decay: `{v.decay_class}` (half-life={v.decay_half_life:.0f} bars)"
                        )
                        st.markdown(
                            f"- Max Factor Corr: `{v.max_factor_corr:.3f}` | "
        ...
        with tab_exec:
            if r["demo_mode"]:
                st.warning(
                    "All signals REJECTED by firewall. "
            ...
            if not all_exec:
                ...
                for ev in all_exec:
                    ...
                    with st.expander(f"{icon} {ev.signal_id} — {ev.verdict}", expanded=True):
                        ...
                        with c1:
                            ...
                            st.metric("Breakeven HR", f"{ev.costs.breakeven_hit_rate:.1%}")

                        with c2:
                            st.markdown("**Prop Firm Feasibility**")
                            st.metric("Kelly f*", f"{ev.prop_firm.kelly_fraction:.3f}")
                            st.metric("Half-Kelly Contracts", ev.prop_firm.half_kelly_contracts)
                            st.metric("MC Ruin Prob", f"{ev.prop_firm.mc_ruin_probability:.2%}")
                            st.metric("Consistency", f"{ev.prop_firm.consistency_score:.1%}")

                        with c3:
                            st.markdown("**Risk Parameters**")
                            st.metric("Max Contracts", ev.risk_parameters.get("max_contracts", 0))
                            st.metric(
                                "Stop Loss", f"{ev.risk_parameters.get('stop_loss_ticks', 0)} ticks"
                            )
                            st.metric("Worst Day", f"${ev.prop_firm.worst_day_pnl:,.2f}")
                            st.metric("Max Trail DD", f"${ev.prop_firm.max_trailing_dd:,.2f}")

                        if ev.veto_reason:
                            st.error(f"**Veto Reason:** {ev.veto_reason}")

                        # Cost waterfall
                        st.plotly_chart(chart_cost_breakdown(ev), use_container_width=True)

                        # Turnover details
                        st.markdown("**Turnover Analysis**")
                        tc1, tc2, tc3 = st.columns(3)
                        with tc1:
                            st.metric("Trades/Day", f"{ev.turnover.get('trades_per_day', 0):.1f}")
                        with tc2:
                            st.metric(
                                "Avg Holding", f"{ev.turnover.get('avg_holding_bars', 0):.1f} bars"
                            )
                        with tc3:
                            st.metric("Flip Rate", f"{ev.turnover.get('flip_rate', 0):.1%}")
                ...
                if portfolio.get("total_signals", 0) > 0:
                    ...
                    with pc3:
                        st.metric("Combined Net Sharpe", f"{portfolio['combined_net_sharpe']:.2f}")
        ...
        with tab_mon:
            ...
            with mc1:
                st.metric("Market Regime", f"{regime_icons.get(regime, '')} {regime}")
            with mc2:
            ...
            if r["health_reports"]:
                ...
                st.dataframe(pd.DataFrame(hdata), use_container_width=True, hide_index=True)
            ...
            if daily:
                if daily.alerts:
                    ...
                    for a in daily.alerts:
                        icons = {"HALT": "🚨", "CRITICAL": "🔴", "WARNING": "⚠️", "INFO": "ℹ️"}
                        st.markdown(
                            f"{icons.get(a.level, '')} **{a.level}** — {a.metric}: {a.message}"
                        )
                else:
