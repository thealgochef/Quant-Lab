...
zones = pd.concat(
    [pd.read_parquet(f) for f in glob.glob(str(OUT / "zones_*.parquet"))], ignore_index=True
)
touch = pd.concat(
    [pd.read_parquet(f) for f in glob.glob(str(OUT / "touches_*.parquet"))], ignore_index=True
)
zones = zones[zones.date.isin(DU)].copy()
touch = touch[touch.date.isin(DU)].copy()
...
lab = en[["k", "label_encoded", "drop_reason", "honest_gross_pts", "session_rth"]].drop_duplicates(
    "k"
)
...
perday = pd.DataFrame(
    {
        ...
        "zones_untouched_to_rth": zg.apply(
            lambda g: ((g.first_straddle_session == "ny_rth") | (g.n_straddles == 0)).sum()
        ),
...
out["perday_first_touch_rth_hist"] = (
    perday.zones_first_touch_rth.value_counts().sort_index().to_dict()
)

# ---- PART 2C: THE KEY NUMBER ----
out["zones_total"] = int(len(zones))
out["zones_with_any_rth_straddle"] = int((zones.n_rth_straddles > 0).sum())
out["zones_first_touch_in_rth (recorded RTH touches)"] = int(
    (zones.first_straddle_session == "ny_rth").sum()
)
out["zones_RTH_swept_but_consumed_overnight"] = int(
    zones.first_straddle_overnight_but_rth_swept.sum()
)
out["pct_RTH_sweeps_consumed"] = float(
    zones.first_straddle_overnight_but_rth_swept.sum() / max(1, (zones.n_rth_straddles > 0).sum())
)
# lookahead
out["lookahead_touches_total"] = int(zones.lookahead_first_before_avail.sum())
out["lookahead_by_first_session"] = (
    zones[zones.lookahead_first_before_avail].first_straddle_session.value_counts().to_dict()
)
...
out["E1_travel"] = {
    ...
    for c in [
        "net_disp30_pts",
        "approach_range30_pts",
        "dist_session_open_pts",
        "dist_rth_open_pts",
    ]
}
...
out["E3_rebound_by_level_type"] = (
    kept.groupby("level_type").reb.agg(["mean", "count"]).to_dict("index")
)
# rebound: raw-extreme self-touch vs not
out["E3_rebound_raw_extreme_vs_not"] = (
    kept.groupby(kept.rep_is_raw_extreme).reb.agg(["mean", "count"]).to_dict("index")
)
