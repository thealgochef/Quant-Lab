...
from alpha_lab.agents.data_infra.ml.engine_decision import TRADE_TICK, bars_et_to_engine, levels_to_engine

DATA_DIR = Path("C:/Users/gonza/Documents/Claude-Quant-Lab/data/databento")
SYM = "NQ"; ET = ZoneInfo("US/Eastern")
OUT = Path(__file__).parent / "out"
util = DashboardUtilityConfig(tp_points=15.0, sl_points=15.0, trap_mfe_min=5.0,
    interaction_window_minutes=5, level_proximity_pts=0.5, bar_type="147t",
    include_approach_features=True, approach_window_minutes=15)


def sess(ts_et: pd.Timestamp) -> str:
    h = ts_et.hour + ts_et.minute / 60.0
    if 9.5 <= h < 16.25: return "ny_rth"
    if 8 <= h < 9.5: return "premarket"
    if 1 <= h < 8: return "london"
    if h >= 18 or h < 1: return "asia"
...
def avail_from(names, td):
    ...
    for n in names:
        nl = n.lower()
        if nl in ("pdh", "pdl"):
            cands.append(pd.Timestamp.combine(td - timedelta(days=1), time(18, 0)).tz_localize(ET))  # day start
        elif nl.startswith("asia"):
            cands.append(pd.Timestamp.combine(td, time(1, 0)).tz_localize(ET))   # asia closes 01:00 ET
        elif nl.startswith("london"):
            cands.append(pd.Timestamp.combine(td, time(8, 0)).tz_localize(ET))   # london closes 08:00 ET
        else:
...
def probe_date(date_str, prev_state):
    ...
    o = bars_et["open"].values; c = bars_et["close"].values
    hi = bars_et["high"].values; lo = bars_et["low"].values
    ...
    for zi, z in enumerate(zones):
        ...
        sess_straddles = [sess(closes_et[i]) for i in straddle_idx]
        ...
        zrows.append({
            "date": date_str, "zone_idx": zi, "rep_price": rep, "side": z.side.value,
            "names": "|".join(z.names), "n_levels": len(z.names),
            "available_from_et": str(af.time()), "n_straddles": len(straddle_idx),
            ...
            "rth_straddle_first_et": str(closes_et[rth_straddles[0]]) if rth_straddles else None,
            ...
            "first_straddle_overnight_but_rth_swept": bool(first_sess not in (None, "ny_rth") and len(rth_straddles) > 0),
        })
        ...
        if first is not None:
            ...
            def win(arr, w):
                ...
                return a
            atr14 = float(np.mean(hi[max(0, loc - 14):loc] - lo[max(0, loc - 14):loc])) if loc >= 1 else np.nan
            rets = np.diff(c[max(0, loc - 31):loc + 1]) if loc >= 1 else np.array([])
            ret_std30 = float(np.std(rets)) if len(rets) > 1 else np.nan
            net_disp30 = float(abs(c[loc] - c[loc - 30])) if loc >= 30 else (float(abs(c[loc] - c[0])) if loc > 0 else 0.0)
            apr = win(hi, 30); aprl = win(lo, 30)
            approach_range30 = float(apr.max() - aprl.min()) if len(apr) else np.nan
            trows.append({
                "date": date_str, "zone_idx": zi, "event_ts": str(first_et),
                "session": first_sess, "level_type": z.names[0], "direction": ("LONG" if z.side.value == "LOW" else "SHORT"),
                "rep_price": rep, "n_levels": len(z.names),
                "lookahead": bool(first_et < af), "rep_is_raw_extreme": bool(is_raw),
                "bar_index": int(loc), "vol_atr14_pts": atr14, "ret_std30_pts": ret_std30,
                "net_disp30_pts": net_disp30, "approach_range30_pts": approach_range30,
                "dist_session_open_pts": float(abs(rep - sess_open_px)),
                "dist_rth_open_pts": float(abs(rep - rth_open_px)) if not np.isnan(rth_open_px) else np.nan,
                "vol_touch": float(vol[loc]), "vol_mean30": float(np.mean(win(vol, 30))) if loc else np.nan,
            })
    return pd.DataFrame(zrows), pd.DataFrame(trows), new_state


def available(start, end):
    out = []; d = date.fromisoformat(start); d1 = date.fromisoformat(end)
    while d <= d1:
        if (DATA_DIR / SYM / d.isoformat()).is_dir(): out.append(d.isoformat())
        d += timedelta(days=1)
    return out
...
def main():
    ...
    ap.add_argument("--warm", type=int, default=8); a = ap.parse_args()
    ...
    for ds in available(warm_start, a.end):
        ...
        if ds in target and zf.exists():
            prev = _get_session_hl_for_date(DATA_DIR, SYM, ds, util, *prev); continue
        try:
            z, t, prev = probe_date(ds, prev)
        except Exception as e:
            sys.stderr.write(f"FAIL {ds}: {e}\n")
            try: prev = _get_session_hl_for_date(DATA_DIR, SYM, ds, util, *prev)
            except Exception: pass
            continue
        if ds in target and z is not None:
            z.to_parquet(zf, index=False)
            (t if t is not None else pd.DataFrame()).to_parquet(OUT / f"touches_{ds}.parquet", index=False)
