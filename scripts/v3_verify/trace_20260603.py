...
def main() -> int:
    ...
    print(
        "scheme:",
        {
            n: f"{w.start}-{w.end}{' +1d' if w.crosses_midnight else ''}"
    ...
    print()
    if asia.empty:
        print("ASIA EMPTY — grouping broken")
        ok = False
    else:
        first = asia.index[0]
    ...
    print(
        f"asia_high/low (builder _slice_session) = {sl_hi} / {sl_lo}  "
        f"match={eng_hi == sl_hi and eng_lo == sl_lo}"
    )
    if not (eng_hi == sl_hi and eng_lo == sl_lo):
        print("!! engine bucket and builder slice disagree on asia H/L")
        ok = False
    ...
    pdh = float(prior_et["high"].max())
    pdl = float(prior_et["low"].min())
    ...
    no_double = (
        last_prior.tz_convert(_ET).time() < pd.Timestamp("18:00").time()
        or last_prior.date() < date.fromisoformat(PRIOR_DAY)
        or (
            last_prior.date() == date.fromisoformat(PRIOR_DAY)
            and last_prior.time() < pd.Timestamp("18:00").time()
        )
    )
    print()
    print(
        f"PDH/PDL for {DAY} = FULL prior trading day {PRIOR_DAY} window "
        f"[{prior_et.index[0]} .. {prior_et.index[-1]}]"
    )
    print(f"   PDH = {pdh}   PDL = {pdl}")
    print(
        f"   prior-day window ends {last_prior} (< 2025-06-02 18:00 ET, so no "
